...
const META = { // comp, export, archivo trace, transform (bbox estable)
  ...
  // mazda3: E1.23 — ref RENOVADA a estilo plano verde; ahora vive en trace-mix/gen-arts-mix
  corolla:  ['CorollaArt', 'COROLLA_TRACE', 'corollaTrace', 'translate(19.5 18.3) scale(0.13)'],
  // yaris: E1.23 — ídem (trace-mix/gen-arts-mix)
...
for (const [key, [comp, exp, trace, tf]] of Object.entries(META)) {
  const M = MODELS[key];
  const isRed = (n) => { const c = M.classes.find(x => x.name === n); return c && c.rgb && c.rgb[0] - c.rgb[1] > 60 && !['amber', 'orange', 'darkorange', 'edge'].includes(n); }; // E1.20: ámbar/edge fuera
