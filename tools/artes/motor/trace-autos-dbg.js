...
(async () => {
  ...
  if (process.env.DBGPX) { for (const pt of process.env.DBGPX.split(';')) { const [dx,dy]=pt.split(',').map(Number); const v=cls[dy*W+dx]; console.log('DBG', pt, 'antes de mayoría 5×5:', v>=0?CLASSES[v].name:'bg'); } }
  ...
  if (process.env.DBGPX) { for (const pt of process.env.DBGPX.split(';')) { const [dx,dy]=pt.split(',').map(Number); const v=cls[dy*W+dx]; console.log('DBG', pt, 'antes de despeckle por compo:', v>=0?CLASSES[v].name:'bg'); } }
  ...
  if (process.env.DBGPX) { for (const pt of process.env.DBGPX.split(';')) { const [dx,dy]=pt.split(',').map(Number); const v=cls[dy*W+dx]; console.log('DBG', pt, 'antes de despeckle de BLANCO:', v>=0?CLASSES[v].name:'bg'); } }
  ...
  if (process.env.DBGPX) { for (const pt of process.env.DBGPX.split(';')) { const [dx,dy]=pt.split(',').map(Number); const v=cls[dy*W+dx]; console.log('DBG', pt, 'antes de E1.17: manchas:', v>=0?CLASSES[v].name:'bg'); } }
  ...
  if (process.env.DBGPX) { for (const pt of process.env.DBGPX.split(';')) { const [dx,dy]=pt.split(',').map(Number); const v=cls[dy*W+dx]; console.log('DBG', pt, 'antes de E1.19: interior:', v>=0?CLASSES[v].name:'bg'); } }
  ...
  if (process.env.DBGPX) { for (const pt of process.env.DBGPX.split(';')) { const [dx,dy]=pt.split(',').map(Number); const v=cls[dy*W+dx]; console.log('DBG', pt, 'antes de mergeInto: clasific:', v>=0?CLASSES[v].name:'bg'); } }
  ...
  if (process.env.DBGPX) { for (const pt of process.env.DBGPX.split(';')) { const [dx,dy]=pt.split(',').map(Number); const v=cls[dy*W+dx]; console.log('DBG', pt, 'antes de bbox + transform:', v>=0?CLASSES[v].name:'bg'); } }
