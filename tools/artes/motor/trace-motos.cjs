...
const MODELS = {
  ...
  xpulse: {
    ...
    fixed: [
      ...
      { from: 'lightgray', to: 'hubdot', box: [643, 648, 675, 679] }, // E1.22: perno del pivote del basculante (se despeckleaba al oscuro)
    ],
...
Object.assign(MODELS, {
  ...
  xr: { // E1.14 (24-ago): RE-MEDIDO todo — las anclas viejas estaban corridas:
    ...
    dilateClasses: { gray: 0 },   // E1.22: rayos de 3-4px salían al doble de grosor (0 = ancho exacto clasificado)
    fixed: [],
    whiteBoxes: [
  ...
  zeta: { // fondo encerrado en whiteBoxes (tras el motor y sobre la llanta
    ...
    classes: [
      ...
      { name: 'hubdot',  rgb: null },  // E1.22: punto de maza TRASERA (142 — isla entre gray y silver, el despeckle la fundía)
      { name: 'hubdot2', rgb: null },  // E1.22: punto de maza DELANTERA (221 — isla clara en mar oscuro)
    ],
    fixed: [
      { from: 'gray',   to: 'hubdot', box: [303, 715, 328, 741] },
      { from: 'silver', to: 'hubdot', box: [303, 715, 328, 741] },
      { from: 'xlight',    to: 'hubdot2', box: [1227, 719, 1252, 744] },
      { from: 'lightgray', to: 'hubdot2', box: [1227, 719, 1252, 744] },
      { from: 'silver',    to: 'hubdot2', box: [1227, 719, 1252, 744] },
    ],
    whiteBoxes: [[400, 380, 1250, 560, 228, 246], [200, 260, 460, 360, 228, 246], [455, 415, 730, 600, 200, 246]],
    shadow: { band: [906, 945], th: 198 },
    order: ['tire', 'dark', 'charcoal', 'gray', 'silver', 'lightgray', 'xlight', 'green', 'white', 'hubdot', 'hubdot2'],
    fills: { green: '#76B233', charcoal: '#3E4244', dark: '#2E3436', tire: '#20282A', gray: '#787A7E', silver: '#A9AAAD', lightgray: '#CCCDD2', xlight: '#DDDEE0', white: '#EDEDED', hubdot: '#8E8F92', hubdot2: '#DDDDDE' },
  },
  ...
  dm: { // la raíz sombreada del guardafangos delantero (199,77,21) caía en
    ...
    classes: [
      ...
      { name: 'silver15',  rgb: [174, 176, 178] }, // E1.20: a 178 empataba con silver2 (187) sobre la tira del amortiguador (185) → se partía en trozos <minPx y el despeckle los cedía al negro
    ...
    mergeInto: { silver15: 'silver2' }, // E1.20b: el rack (174-184) se partía entre ambas anclas
...
(async () => {
  ...
  {
    const SPECIAL = new Set(['red', 'darkred', 'green', 'orange', 'darkorange', 'teal', 'darkteal', 'blue', 'amber', 'white', 'taillight', 'tailred', 'spring', 'navy', 'springsilver', 'hubdot', 'hubdot2', 'ring']);
    const GRAY = CLASSES.map((c, i) => (!SPECIAL.has(c.name) && c.rgb ? i : -1)).filter(i => i >= 0);
    const MIN_PX = M.minPx || 150;
    // E1.20: clases que se fusionan al mismo destino cuentan como UNA (ver trace-autos)
  ...
  // E1.22: dilateClasses = override POR CLASE (patrón dilateRed E1.20b) —
  // los rayos finos del XR (3-4px en la ref) salían al DOBLE de grosor
  // con la dilatación estándar de 2px por lado.
  ...
  const traceOne = (name) => new Promise((res, rej) => {
    ...
    m = dilate(m, (M.dilateClasses && M.dilateClasses[name] !== undefined) ? M.dilateClasses[name] : 2);
