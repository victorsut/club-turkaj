...
/** Genera código de tarjeta digital con prefijo según tier */
export function makeCardCode(tierName, numericPart) {
  const prefix = CARD_PREFIX[tierName] || 'CTOD';
  const num = String(numericPart).padStart(5, '0');
  return `${prefix}-${num}`;
}

/** Extrae la parte numérica de cualquier código CT*D-XXXXX */
export function cardCodeNumber(code) {
  if (!code) return 0;
  const match = code.match(/^CT[OPB]D-(\d+)$/);
  return match ? parseInt(match[1], 10) : 0;
}

/** Actualiza el prefijo de un código existente al tier actual */
export function updateCardPrefix(code, tierName) {
  const num = cardCodeNumber(code);
  if (num > 0) return makeCardCode(tierName, num);
  // Si no tiene formato válido, conservar el original
  return code;
}
