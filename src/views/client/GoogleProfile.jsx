...
export default function GoogleProfile(ctx) {
  ...
  const doFinish = () => {
    ...
    // Generate card code and save to Supabase (nuevo miembro siempre empieza en ORO)
    ...
    if (sb && sbConnected) {
      ...
      sb.from('members').insert(memberData).select().then(async (r) => {
        if (r.error) { console.error('[Registro] Error insert member:', r.error); return; }
        if (r.data?.[0]) {
          const dbId = r.data[0].id;
          setMe(p => ({ ...p, id: dbId }));

          // Insert physical card and link back to member
          ...
          if (cardRes.data?.[0]) {
            ...
            console.log('[Registro] \u2705 Tarjeta', fallbackCard, 'vinculada a miembro', dbId);
          } else {
            console.error('[Registro] Error insert card:', cardRes.error);
          }

          sb.from('activity_log').insert({ member_id: dbId, activity_type: 'registro', description: 'Registro vía Google OAuth', points_change: totalPts });
        }
      });
    }
  };
