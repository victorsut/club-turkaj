...
// ── Fuera de GoogleProfile para evitar remount en cada render ──
    ...
    <div style={{ display: 'flex', alignItems: 'center', marginBottom: 28 }}>
      {steps.map((label, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'center', flex: i < steps.length - 1 ? 1 : 'none' }}>
          ...
          {i < steps.length - 1 && (
            <div style={{ flex: 1, height: 2, background: i < idx ? '#FBBC04' : '#E0E0E0', margin: '0 6px', marginBottom: 18 }} />
          )}
        </div>
      ))}
    </div>
    ...
    <div style={{ background: '#FFF8E1', borderRadius: 14, padding: '12px 16px', marginBottom: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      <div>
        ...
        <div style={{ fontSize: 11, color: '#9E9E9E', marginTop: 2 }}>
          Base {base} + opcionales {optional} + vehículos {vehicles}
        </div>
      </div>
...
function Field({ icon, placeholder, fieldKey, type, inputMode, maxLen, optional,
                 regProfile, setRegProfile, clearAuthErr, regOptional, maxBday }) {
...
export default function GoogleProfile(ctx) {
  ...
  // Props comunes para Field
  const fieldProps = { regProfile, setRegProfile, clearAuthErr, regOptional, maxBday };
  ...
  const doFinish = async () => {
    ...
    const updated = {
      ...
      bday:  bdayStored,
      nit:   regProfile.nit   || '',
      points: totalPts, cardId: fallbackCard,
    };
    setMe(updated);
    ...
    if (sb && sbConnected) {
      ...
      if (memberErr) { console.error('[Reg] insert error:', memberErr); return; }
      ...
      if (vehicles.length > 0) {
        await sb.from('activity_log').insert({
          member_id: dbId, activity_type: 'registro_vehiculos',
          description: `${vehicles.length} vehículo(s) registrado(s) · +${vehiclePts} pts`,
          points_change: vehiclePts, metadata: { vehicles },
        });
      }
    ...
    return (
      <div style={{ padding: '40px 24px 120px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 20 }}>
          ...
          <Field {...fieldProps} icon="🎂" placeholder="Fecha de nacimiento (opcional)" fieldKey="bday" type="date" optional />
      ...
      <div style={{ padding: '40px 24px 120px' }}>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ fontSize: 13, color: '#9E9E9E' }}>
            Ganás <strong style={{ color: '#4CAF50' }}>+{VEHICLE_PTS} pts</strong> por cada vehículo registrado.
          </div>
        </div>
        {authError && (
          <div style={{ background: '#FFEBEE', color: '#C62828', padding: '10px 14px', borderRadius: 12, fontSize: 12, fontWeight: 700, marginBottom: 16, textAlign: 'center' }}>{authError}</div>
          ...
          <div style={{ background: '#FFF8E1', borderRadius: 16, padding: 16, marginBottom: 16, border: '2px solid #FBBC04' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => { setAddingVehicle(false); setNewPlate(''); clearAuthErr(); }}
                style={{ flex: 1, padding: 12, borderRadius: 12, border: '2px solid #eee', background: '#fff', color: '#9E9E9E', fontFamily: "'DM Sans'", fontWeight: 700, cursor: 'pointer', fontSize: 13 }}>
                Cancelar
              </button>
              <button onClick={addVehicle}
                style={{ flex: 2, padding: 12, borderRadius: 12, border: 'none', background: '#FBBC04', color: '#0D0D0D', fontFamily: "'DM Sans'", fontWeight: 900, cursor: 'pointer', fontSize: 13 }}>
                + Agregar
              </button>
            </div>
          </div>
        ...
        {vehicles.length === 0 && !addingVehicle && (
          <div style={{ textAlign: 'center', fontSize: 12, color: '#9E9E9E', marginBottom: 16 }}>
            Podés omitir este paso si no querés registrar vehículos ahora.
          </div>
        )}
        <PtsCard total={totalPts} base={cfg.regBase || 15} optional={optFields * regOptional} vehicles={vehiclePts} />
