...
export default function GoogleProfile(ctx) {
  ...
  const doFinish = () => {
    ...
    const fallbackCard = 'CTOD-' + String(Date.now()).slice(-5);
    const updated = {
    ...
    if (sb && sbConnected) {
      ...
      sb.from('members').insert(memberData).select().then(r => {
        if (r.data?.[0]) {
          const dbId = r.data[0].id;
          setMe(p => ({ ...p, id: dbId }));
          sb.from('physical_cards').insert({ member_id: dbId, card_code: fallbackCard, card_type: 'digital', tier: 'ORO' });
