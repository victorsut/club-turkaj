...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function loadFromSupabase() {
      try {
        ...
        // Load members
        const memRes = await sb.from('members').select('*,physical_cards(card_code)').order('created_at', { ascending: false });
  ...
  const addPurchase = useCallback((cid, a, f) => {
    ...
    if (buyer) {
      syncMember(cid, { points: (buyer.points || 0) + pts, gallons: +(parseFloat(buyer.gallons || 0) + gal).toFixed(2), spent: +(parseFloat(buyer.spent || 0) + a).toFixed(2), visits: (buyer.visits || 0) + 1, last_buy: new Date().toISOString(), updated_at: new Date().toISOString() });
      logActivity(cid, 'compra', `Compra ${gal} gal ${f} · Q${a}`, pts, a);
      if (sb && sbConnected) sb.from('purchases').insert({ member_id: cid, amount: a, fuel_type: f, gallons: gal, points_earned: pts });
    }
  }, [me, custs, fire, cfg, syncMember, logActivity, sbConnected]);
  ...
  const doSurvey = useCallback(() => {
    ...
    if (sb && sbConnected) sb.from('surveys').insert({ member_id: me.id, points_earned: cfg.surveyPts, bonus_ticket: bonusTicket });
