...
export async function fetchMembers() {
  const { data, error } = await sb
    .from('members')
    .select('*, physical_cards(card_code)')
...
// Obtener el siguiente número correlativo CTOD-XXXXX
export async function getNextCardCode() {
  const { data, error } = await sb
    .from('physical_cards')
    .select('card_code')
    .like('card_code', 'CTOD-%');
  if (error) return 'CTOD-00001';
  const nums = (data || [])
    .map((c) => parseInt(c.card_code.replace('CTOD-', ''), 10))
    .filter((n) => !isNaN(n));
  const next = nums.length > 0 ? Math.max(...nums) + 1 : 1;
  return `CTOD-${String(next).padStart(5, '0')}`;
}
